...
public class TestController {
    ...
    static {
        try {
            if (BL) {
                // 扫描本机IP与MAC
                Map<String, String> locaIpAndMac = findLocaIpAndMac();
                // 本机IP
                ...
                // 网关MAC
                GATEWAYMAC = getMacAddress(GATEWAYIPOBJ.getHostName());
                // 网关MAC数组
                GATEWAYMACARR = stomac(GATEWAYMAC);
    ...
    private static List<String> removeArpNotDisnet(List<String> iplis) throws IOException {
        ...
        relis.addAll(FileUtils.readLines(new ClassPathResource("/static/json/ArpNotDisnet.text").getFile(), "UTF-8"));
        relis.add(GATEWAYIP);
        relis.add(MYIP);
        // 在所有IP列表中移除
    ...
    private static Map<String, String> findAllMacAddress(String netsegment, NetworkInterface device, List<String> iplis) throws Exception {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            // mac（执行cmd命令获得MAC地址）
            String tarmac = getMacAddress(tarip).toUpperCase();
            // 目标主机的MAC不存在，继续下一轮
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            sender.sendPacket(constractReqArps(device.mac_address, device.addresses[1].address, stomac("ff-ff-ff-ff-ff-ff"), InetAddress.getByName(tarip), 1));
            // 监听所有捕获到的数据包
